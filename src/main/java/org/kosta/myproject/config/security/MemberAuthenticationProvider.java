...
public class MemberAuthenticationProvider implements AuthenticationProvider{	
    ...
    @Autowired
	public MemberAuthenticationProvider(MemberService memberService, BCryptPasswordEncoder passwordEncoder) {
		super();
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {		
		...
		//2.사용자 정보 DB로부터 조회
		String email = authentication.getName();//사용자가 로그인시 입력한 ID 반환 		
		MemberDTO member = memberService.findMemberById(email);
		if(member == null){
		...
		//4.사용자 권한 조회
		List<AuthorityDTO> list = memberService.findAuthorityByUsername(email);
		if(list.size() == 0){
			throw new UsernameNotFoundException("권한이 없습니다.");
		}
		
		List<SimpleGrantedAuthority> authorities = new ArrayList<SimpleGrantedAuthority>();		
		for(AuthorityDTO au : list){ // ROLE_ 형식의 db 정보가 아니라면 이 시점에 ROLE_ 를 접두어로 추가한다
