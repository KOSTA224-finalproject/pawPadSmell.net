...
//		memberDTO = (MemberDTO) authentication;
	...
	public String Upload(HttpSession session, Model model, Authentication authentication, MemberDTO memberDTO,
			String pageNo, MyPageDTO mypageDTO, MultipartFile file) throws IllegalStateException, IOException {
		memberDTO = (MemberDTO) authentication.getPrincipal();
		if (file.isEmpty()) {
			...
			boardMapper.myPageWrite1(mypageDTO);
	...
	@ResponseBody
	public void getProfile(HttpSession session, Authentication authentication, MyPageDTO mypageDTO, MultipartFile file)
	...
	@RequestMapping("member/memberpage/{memberId}")
	public String memberInfo(Authentication authentication, Model model, String pageNo, MyPageDTO mypageDTO,
			@PathVariable int memberId, MemberDTO memberDTO) {
		...
		model.addAttribute("memberpage", mypageDTO);
		...
		model.addAttribute("list",
				boardMapper.getAllMemberInfo(memberId, pagingBean.getStartRowNumber(), pagingBean.getEndRowNumber()));
		// List<BoardDTO> getAllMemberInfo(int memberId,int getStartRowNumber, int
		// getEndRowNumber );//회원 정보 조회 - 작성한 게시글 리스트

		return "member/memberInfo.tiles2";
	}
